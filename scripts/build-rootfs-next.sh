...
# =========================================================
# ubuntu-baseのダウンロード（固定パスから最新のポイントリリースを自動取得）
# =========================================================
...
# ubuntu-base-xx.xx.xx-base-arm64.tar.gz を自動マッチングし、最新版を取得する
...
# ==================== 日本語ローカライズ環境 ====================
chroot "${CHROOT_DIR}" apt install -y locales language-pack-ja fonts-noto-cjk

# 日本語ロケールの生成
chroot "${CHROOT_DIR}" locale-gen ja_JP.UTF-8

# システム全体のデフォルト言語を日本語に設定
chroot "${CHROOT_DIR}" update-locale LANG=ja_JP.UTF-8 LC_ALL=ja_JP.UTF-8

# 環境変数への書き込み
echo "LANG=ja_JP.UTF-8" >> "${CHROOT_DIR}/etc/environment"
echo "LC_ALL=ja_JP.UTF-8" >> "${CHROOT_DIR}/etc/environment"

# =========================================================
# 必須サービスの有効化（chroot環境では手動で有効化する必要がある）
# =========================================================
...
# x86用独立GPU/計算カードのファームウェアを削除し、rootfsのサイズを縮小する
...
# =======apt update実行時の上書きを防ぐため、firmwareをロック（hold）する=======
...
# =========================================================
# cloud-initによるホスト名の上書きを禁止（Ubuntu 26.04では必須）
# =========================================================
...
# =========================================================
# 拡張：rootでのシリアルポート（シリアルコンソール）ログイン設定
# =========================================================
...
chroot ${CHROOT_DIR} ln -sf /usr/share/zoneinfo/Asia/Tokyo /etc/localtime
echo "Asia/Tokyo" > ${CHROOT_DIR}/etc/timezone
